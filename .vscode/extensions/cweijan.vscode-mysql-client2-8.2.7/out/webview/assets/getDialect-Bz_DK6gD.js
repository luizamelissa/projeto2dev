...
-- RENAME TO new_name
;`}grantRole(e){return`GRANT ${e} TO role_name$1;`}}class d{constructor(e){this.param=e,this.afterTablePrefix=`ALTER TABLE "${e.table}"`,this.afterColumnPrefix=`${this.afterTablePrefix} ALTER COLUMN "${e.columnName}"`}genAlterSQL(){const{columnName:e,newColumnName:a}=this.param;return new i("",`
`).append(this.changeTypePart()).append(this.notNullPart()).append(this.defaultPart()).append(this.otherPart()).append(this.commentPart()).if(e!=a,`${this.afterTablePrefix} RENAME COLUMN "${e}" TO "${a}";`).toString()}changeTypePart(){const{oldRow:e,columnType:a}=this.param;return e.type==a?"":`${this.afterColumnPrefix} TYPE ${a};`}notNullPart(){const{oldRow:e,isNotNull:a}=this.param;return e.isNotNull==a?"":`${this.afterColumnPrefix} ${a?"SET NOT NULL":"DROP NOT NULL"};`}otherPart(){return null}defaultPart(){const{oldRow:e,defaultValue:a,columnType:t}=this.param;return e.defaultValue==a?"":`${this.afterColumnPrefix} ${u(a)?"DROP DEFAULT":`SET DEFAULT ${L(a,t)}`};`}commentPart(){return""}}class S extends N{showVersion(){return""}createIndex(e){return`ALTER TABLE ${e.table} ADD ${e.type||"key"} ("${e.column||"[column]"}")`}dropIndex(e,a){return`DROP INDEX "${a}"`}addColumn(e,a){const t=a?` AFTER "${a}"`:"";return`ALTER TABLE ${e}
...
GRANT ALL PRIVILEGES ON *.* TO 'username'@'%' WITH GRANT OPTION;`}updateUser(e){return`update mysql.user set
    ...
    where User = '${e}';
FLUSH PRIVILEGES;
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")`}updateColumn(e,a){var A;const{name:t,type:r,comment:n,nullable:E,defaultValue:s,extra:o,character_set_name:c,collation_name:_}=a;return new i(`ALTER TABLE ${e}`).append(`
	CHANGE ${t} ${t} ${r}`).if(c,`CHARACTER SET ${c}`).if(_,`COLLATE ${_}`).if(E!="YES","NOT NULL").if((A=o==null?void 0:o.toLowerCase())==null?void 0:A.includes("auto_increment"),"AUTO_INCREMENT").if(n,`COMMENT '${n}'`).if(s,`DEFAULT ${s=="CURRENT_TIMESTAMP"?s:`'${s}'`}`).toString()}updateColumnSql(e){return new d(e).genAlterSQL()}updateTable(e){const{table:a,newTableName:t,comment:r,newComment:n}=e;let E="";return n&&n!=r&&(E=`COMMENT ON TABLE "${a}" IS '${n}';`),t&&a!=t&&(E+=`RENAME TABLE "${a}" TO "${t}";`),E}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE "',table_schema,'"."',TABLE_NAME, '";') trun FROM INFORMATION_SCHEMA.TABLES where  table_schema ='${e}' and TABLE_TYPE<>'VIEW';`}createDatabase(e){return"CREATE DATABASE $1;"}showTableSource(e,a){return`SHOW CREATE TABLE ${e}.${a};`}showViewSource(e,a,t){return`SHOW CREATE VIEW database.${a};`}showColumns(e,a){return`SELECT COLUMN_NAME name,DATA_TYPE type, IS_NULLABLE nullable
...
BEGIN
    $2
    return 0;
END;`}}class w extends N{showTables(e,a,t){return null}addColumn(e,a){return null}showColumns(e,a,t){return null}showViews(e,a){return null}buildPageSql(e,a,t){return null}createDatabase(e){return null}}class g extends N{showVersion(){return`select RTRIM(SUBSTR(REPLACE(banner,'Oracle Database ',''),1,3)) "server_version" from v$version where rownum=1`}showAllForeignKeys(){return`SELECT 
...
ALTER USER $1 quota unlimited on USERS;
        `}updateUser(e){return`ALTER USER ${e} IDENTIFIED BY [new_password]`}updateColumn(e,a){const{name:t,type:r,comment:n,nullable:E,defaultValue:s}=a;return`-- change column type
ALTER TABLE ${e} MODIFY ${t} ${r};
-- change column name
ALTER TABLE ${e} RENAME COLUMN ${t} TO [newColumnName];
COMMENT ON COLUMN ${e}.${t} IS '${n||""}'`}updateColumnSql(e){const{columnName:a,columnType:t,newColumnName:r,comment:n,defaultValue:E,table:s,isNotNull:o,oldRow:c}=e;return new i("","").if(t!=c.type,`ALTER TABLE "${s}" MODIFY "${a}" ${t};`).if(o!=c.isNotNull,`
ALTER TABLE "${s}" MODIFY "${a}"${o?"NOT NULL":"NULL"};`).if(E!=null&&E!=c.defaultValue,`
ALTER TABLE "${s}" MODIFY "${a}" DEFAULT ${E!=null&&E.match(/(:|nextval)/i)?E:`'${E==null?void 0:E.replace(/(^'|'$)/g,"")}'`};`).if(n&&n!=c.comment,`
COMMENT ON COLUMN "${s}"."${a}" is '${n}';`).if(a!=r,`
ALTER TABLE "${s}" RENAME COLUMN "${a}" TO "${r}";`).toString()}showUsers(){return'SELECT username "user" FROM all_users'}pingDataBase(e){return e?`ALTER SESSION SET current_schema = "${e}"`:null}updateTable(e){const{table:a,newTableName:t,comment:r,newComment:n}=e;let E="";return n&&n!=r&&(E=`COMMENT ON TABLE "${a}" IS '${n}';`),t&&a!=t&&(E+=`ALTER TABLE "${a}" RENAME TO "${t}"`),E}truncateDatabase(e){return`SELECT 'TRUNCATE TABLE "' || owner || '"."' || object_name || '";' trun FROM all_objects where  owner ='${e}' and object_type ='TABLE'`}createDatabase(e){return"CREATE USER $1 IDENTIFIED BY password$2;"}showViewSource(e,a,t){return t?`select QUERY "Create View" from ALL_MVIEWS where OWNER='${e}' and mview_name='${a}'`:`SELECT DBMS_METADATA.GET_DDL('VIEW', '${a}', '${e}') AS VIEW_DDL FROM DUAL;`}showProcedureSource(e,a){return`SELECT text "Create Procedure"
        FROM all_source
       WHERE owner = '${e}'
         AND name  = '${a}'
...
-- IDENTIFIED GLOBALLY
;`}grantRole(e){return`GRANT ${e} TO role_name$1;`}}class y extends g{showVersion(){return`SELECT REPLACE(banner,'DM Database Server 64 ','') "server_version"  FROM v$version where rownum=1;`}createIndex(e){const{table:a,column:t="$2"}=e;return`CREATE INDEX ${`${a}_${t}`} ON ${a}(${t});`}dropIndex(e,a){return`DROP INDEX ${a};`}showIndex(e,a){return`SELECT COLUMN_NAME "column_name",INDEX_NAME "index_name" from all_IND_COLUMNS WHERE TABLE_OWNER='${e}' and TABLE_NAME='${a}';`}processList(){return"SELECT SESS_ID,STATE,SQL_TEXT FROM v$sessions"}addColumn(e,a){return`ALTER TABLE ${e} 
    ADD COLUMN $1 $2;
COMMENT ON COLUMN ${e}.$3 IS 'comment$4';`}createUser(){return"CREATE USER $1 IDENTIFIED BY [password]$2;"}updateUser(e){return`ALTER USER ${e} IDENTIFIED BY [new_password];`}updateColumn(e,a){const{name:t,type:r,comment:n,nullable:E,defaultValue:s}=a;return`-- change column type
ALTER TABLE ${e} MODIFY ${t} ${r};
-- change column name
ALTER TABLE ${e} RENAME COLUMN ${t} TO [newColumnName];
COMMENT ON COLUMN ${e}.${t} IS '${n||""}';`}updateColumnSql(e){const{columnName:a,columnType:t,newColumnName:r,comment:n,defaultValue:E,table:s,isNotNull:o,oldRow:c}=e;return new i("","").if(t!=c.type,`ALTER TABLE "${s}" MODIFY "${a}" ${t};`).if(o!=c.isNotNull,`
...
ALTER TABLE "${s}" MODIFY "${a}" DEFAULT ${E!=null&&E.match(/(:|nextval)/i)?E:`'${E==null?void 0:E.replace(/(^'|'$)/g,"")}'`};`).if(n&&n!=c.comment,`
COMMENT ON COLUMN "${s}"."${a}" is '${n}';`).if(a!=r,`
ALTER TABLE "${s}" RENAME COLUMN "${a}" TO "${r}";`).toString()}showUsers(){return'SELECT username "user" FROM all_users'}pingDataBase(e){return e?`set SCHEMA ${e}`:null}updateTable(e){const{table:a,newTableName:t,comment:r,newComment:n}=e;let E="";return n&&n!=r&&(E=`COMMENT ON TABLE "${a}" IS '${n}';`),t&&a!=t&&(E+=`ALTER TABLE "${a}" RENAME TO "${t}"`),E}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE \`',table_schema,'\`.\`',TABLE_NAME, '\`;') trun FROM INFORMATION_SCHEMA.TABLES where  table_schema ='${e}' and TABLE_TYPE<>'VIEW';`}createDatabase(e){return"CREATE USER [name] IDENTIFIED BY [password];"}showViewSource(e,a){return`CALL SP_VIEWDEF('${e}', '${a}');`}showProcedureSource(e,a){return`SELECT LISTAGG(text) within group(order by line) "Create Procedure"
        FROM all_source
       WHERE owner = '${e}'
         AND name  = '${a}'
...
BEGIN
    $3
    return x*2;
END;`}}class M extends N{showVersion(){return"select @@version server_version;"}showAllForeignKeys(){return`SELECT 
...
GRANT ALL PRIVILEGES ON *.* TO '$1'@'%' WITH GRANT OPTION;`}updateUser(e){return`update mysql.user set 
    ...
    where User = '${e}';
FLUSH PRIVILEGES;
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")`}updateColumn(e,a){var n,E;const{nullable:t,extra:r}=a;return this.updateColumnSql({table:e,...a,isNotNull:t!="YES",isAutoIncrement:(E=(n=r==null?void 0:r.toLowerCase())==null?void 0:n.includes)==null?void 0:E.call(n,"auto_increment")})}updateColumnSql(e){const{table:a,tableMeta:t,name:r,columnName:n=r,type:E,unsigned:s,zerofill:o,useCurrentTimestamp:c,isNotNull:_,isAutoIncrement:A,comment:m,defaultValue:R,character_set_name:C,collation_name:O}=e,D=(E==null?void 0:E.match(/char|text|set|enum|blob|binary/i))&&(t==null?void 0:t.collation)!=O;return new i(`ALTER TABLE \`${a}\``).append(`
	CHANGE \`${n}\` \`${r}\` ${E}`).if(s=="1","UNSIGNED").if(o=="1","ZEROFILL").if(c,"ON UPDATE CURRENT_TIMESTAMP").if(D&&C,`CHARACTER SET ${C}`).if(D&&O,`COLLATE ${O}`).if(_,"NOT NULL").if(A,"AUTO_INCREMENT").if(m,`COMMENT '${m}'`).if(u(R)&&!_,"DEFAULT NULL").if(!u(R),`DEFAULT ${R=="CURRENT_TIMESTAMP"?R:`${L(R,E)}`}`).append(";").toString()}showCollations(){return'select DEFAULT_COLLATE_NAME "name",CHARACTER_SET_NAME "charset",DESCRIPTION "description" from information_schema.CHARACTER_SETS ORDER BY name;'}showChecks(e,a){return`SELECT
    ...
    WHERE
        ...
        AND tc.CONSTRAINT_NAME = cc.CONSTRAINT_NAME;`}showUsers(){return"SELECT user user,host host FROM mysql.user;"}pingDataBase(e){return e?`use \`${e}\``:null}updateTable(e){const{table:a,newTableName:t,comment:r,newComment:n,collation:E,newCollation:s}=e;let o="";return n&&n!=r&&(o=`ALTER TABLE \`${a}\` COMMENT = '${n}';`),s&&s!=E&&(o+=`ALTER TABLE \`${a}\` collate = '${s}';`),t&&a!=t&&(o+=`ALTER TABLE \`${a}\` RENAME TO \`${t}\`;`),o}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE \`',table_schema,'\`.\`',TABLE_NAME, '\`;') trun FROM INFORMATION_SCHEMA.TABLES where  table_schema ='${e}' and TABLE_TYPE<>'VIEW';`}createDatabase(e){return`CREATE DATABASE $1
...
BEGIN
    $3
    return 0;
END;`}}class P extends M{showVersion(){return"select replace(@@version,'-MariaDB','') server_version;"}}class Y extends w{showVersion(){return"show version"}showDatabases(){return"show dbs"}buildPageSql(e,a,t){return`db('${e}').collection('${a}').find({}).limit(${t}).toArray()`}showIndex(e,a){return`db('${e}').collection('${a}').indexes();`}createIndex(e){const{database:a,table:t,column:r}=e;return`db('${a}').collection('${t}').createIndex({ ${r||"column_name"}: 1 });`}createDatabase(e){return'db("db_name").createCollection("collection")'}dropDatabase(e){return`db("${e}").dropDatabase()`}}class W extends N{showVersion(){return"SELECT CAST(SERVERPROPERTY('ProductVersion') AS SYSNAME)+' '+CAST(SERVERPROPERTY('Edition') AS SYSNAME) AS server_version"}showAllForeignKeys(){return`SELECT 
...
-- ALTER ROLE db_ddladmin ADD MEMBER userName$3;`}updateColumn(e,a){const{name:t,type:r,comment:n,nullable:E,defaultValue:s}=a,o=E=="YES"?"NULL":"NOT NULL";return`-- change column type
ALTER TABLE ${e} 
    ALTER COLUMN ${t} ${r} ${o};
-- change column name
EXEC sp_rename '${e}.${t}', '${t}', 'COLUMN';`}updateColumnSql(e){const{columnName:a,columnType:t,defaultValue:r,newColumnName:n,comment:E,oldRow:s,isNotNull:o,schema:c,table:_}=e,A=o?"NOT NULL":"NULL";return new i(`ALTER TABLE "${c}"."${_}" ALTER COLUMN ${a} ${t} ${A};`,`
`).if((s==null?void 0:s.defaultValue)&&s.defaultValue!=r,()=>`DECLARE @constraintName NVARCHAR(128)
         ...
         END;`).if(!u(r)&&s.defaultValue!=r,()=>`ALTER TABLE "${c}"."${_}" ADD CONSTRAINT DF_${_}_${a} DEFAULT ${L(r,t)} FOR ${a};`).if(a!=n,()=>`EXEC sp_rename '${c}.${_}.${a}' , '${n}', 'COLUMN';`).if(E!=s.comment,()=>`EXEC ${s.comment?"sp_updateextendedproperty":"sp_addextendedproperty"} 
@level0type = N'Schema', @level0name = '${c}',
@level1type = N'Table', @level1name = '${_}', 
@level2type = N'Column', @level2name = '${n}',
@name = N'MS_Description', @value = N'${E}';`).toString()}showDatabaseUsers(){return"SELECT name [user] from sys.database_principals WHERE type IN ('S', 'U', 'G','E') "}updateTable(e){const{schema:a,table:t,newTableName:r,comment:n,newComment:E}=e;let s="";return E&&E!=n&&(s=`EXEC ${n?"sp_updateextendedproperty":"sp_addextendedproperty"} 
@level0type = N'Schema', @level0name = '${a}',
@level1type = N'Table', @level1name = '${t}', 
@name = N'MS_Description', @value = N'${E}';`),r&&t!=r&&(s+=`sp_rename '${t}', '${r}';`),s}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE [',table_schema,'].[',TABLE_NAME, '];') trun FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE'  AND TABLE_SCHEMA='${e}'`}createDatabase(e){return`create database ${e||"[name]"}`}showViewSource(e,a){return`SELECT definition 'Create View' FROM sys.sql_modules WHERE object_id = OBJECT_ID('${e}.${a}');`}showProcedureSource(e,a){return`SELECT definition 'Create Procedure','${e}.${a}' "Procedure" FROM sys.sql_modules WHERE object_id = OBJECT_ID('${e}.${a}');`}showFunctionSource(e,a){return`SELECT definition 'Create Function','${e}.${a}' "Function" FROM sys.sql_modules WHERE object_id = OBJECT_ID('${e}.${a}');`}showTriggerSource(e,a){return`SELECT definition 'SQL Original Statement','${e}.${a}' "Trigger" FROM sys.sql_modules WHERE object_id = OBJECT_ID('${e}.${a}');`}showColumns(e,a){return["information_schema","sys"].includes(e==null?void 0:e.toLowerCase())?`SELECT
            name,
...
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO $1;`}updateUser(e){return`ALTER USER ${e} WITH PASSWORD 'new_password';`}updateColumn(e,a){const{name:t,type:r,comment:n,nullable:E,defaultValue:s}=a;return`-- change column type
ALTER TABLE ${e} 
    ALTER COLUMN ${t} TYPE ${r};
-- change column name
ALTER TABLE ${e} 
...
COMMENT ON COLUMN ${e}.${t} IS '${n||"comment"}';`}updateColumnSql(e){return new p(e).genAlterSQL()}showUsers(){return'SELECT usename "user" from pg_user '}showAllForeignKeys(){return`SELECT
...
WHERE c.contype = 'f' and sch.nspname = '${e}' and tbl.relname = '${a}'`}pingDataBase(e){return e?`set search_path to '${e}';`:null}updateTable(e){const{table:a,newTableName:t,comment:r,newComment:n}=e;let E="";return n&&n!=r&&(E=`COMMENT ON TABLE "${a}" IS '${n}';`),t&&a!=t&&(E+=`ALTER TABLE "${a}" RENAME TO "${t}";`),E}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE "',TABLE_NAME, '";') trun FROM INFORMATION_SCHEMA.TABLES WHERE  table_schema ='${e}' AND table_type='BASE TABLE';`}createDatabase(e){return"CREATE DATABASE $1"}showViewSource(e,a,t){return t?`SELECT CONCAT('CREATE MATERIALIZED VIEW ',matviewname,'
AS
(',regexp_replace(definition,';$',''),')') "Create View"
    ,matviewname "table_name",'definition' "view_definition" from pg_matviews
    WHERE schemaname='${e}' and matviewname='${a}';`:`SELECT CONCAT('CREATE VIEW ',table_name,'
AS
(',regexp_replace(view_definition,';$',''),')') "Create View"
    ,table_name,view_definition from information_schema.views 
    where table_schema='${e}' and table_name='${a}'`}showMaterialViewSource(e,a){return`SELECT CONCAT('CREATE MATERIALIZED VIEW ',matviewname,'
AS
(',regexp_replace(definition,';$',''),')') "Create View"
    ,matviewname "table_name",'definition' "view_definition" from pg_matviews
    WHERE schemaname='${e}' and matviewname='${a}';`}showProcedureSource(e,a){return`select pg_get_functiondef('${e}.${a}' :: regproc) "Create Procedure",'${a}' "Procedure";`}showTriggerSource(e,a){return`select pg_get_triggerdef(oid) "SQL Original Statement",'${e}.${a}' "Trigger" from pg_trigger where tgname = '${a}';`}showColumns(e,a,t){return`SELECT 
          ${t?"c.*,":""}
          c.COLUMN_NAME "name", 
...
;`}grantRole(e){return`GRANT ${e} TO role_name$1;`}}class I extends ${showAllForeignKeys(){return`SELECT
  ...
  ORDER BY ord_position;`}showProcedures(e){return`SELECT ROUTINE_NAME "ROUTINE_NAME" FROM information_schema.routines WHERE ROUTINE_SCHEMA = '${e}' and ROUTINE_TYPE='PROCEDURE' ORDER BY ROUTINE_NAME ASC`}}class x extends I{showVersion(){return"SELECT VERSION() as server_version"}createIndex(e){return null}variableList(){return"SHOW ALL"}statusList(){return`SELECT
        'db_numbackends' AS db,
        pg_stat_get_db_numbackends(datid) AS status
      FROM
        pg_stat_database
...
AS $$
    SELECT 1
$$ LANGUAGE sql;`}}class v extends N{showVersion(){return'select CURRENT_VERSION() as "server_version";'}createIndex(e){let a=`${e.type||"key"}`;return a.match(/BTREE/i)&&(a="key"),`ALTER TABLE ${e.table} ADD ${a} (\`${e.column||"$1"}\`)`}dropIndex(e,a){return`ALTER TABLE ${e} DROP INDEX \`${a}\``}showIndex(e,a){return""}addColumn(e,a){const t=a?` AFTER \`${a}\``:"";return`ALTER TABLE ${e} 
    ADD COLUMN $1 [type]$2 COMMENT '$3'${t};`}createUser(){return`CREATE USER '$1'@'%' IDENTIFIED BY 'password$2';
-- Grant select privilege to all databases;
GRANT SELECT ON *.* TO '$1'@'%' WITH GRANT OPTION;
-- Grant all privileges to all databases;
GRANT ALL PRIVILEGES ON *.* TO '$1'@'%' WITH GRANT OPTION;`}updateColumn(e,a){var m;const{name:t,type:r,comment:n,nullable:E,defaultValue:s,extra:o,character_set_name:c,collation_name:_}=a,A=E!="YES";return new i(`ALTER TABLE ${e}`).append(`
	CHANGE ${t} ${t} ${r}`).if(c,`CHARACTER SET ${c}`).if(_,`COLLATE ${_}`).if(A,"NOT NULL").if((m=o==null?void 0:o.toLowerCase())==null?void 0:m.includes("auto_increment"),"AUTO_INCREMENT").if(n,`COMMENT '${n}'`).if(u(s)&&!A,"DEFAULT NULL").if(!u(s),`DEFAULT ${s=="CURRENT_TIMESTAMP"?s:`'${B(s)}'`}`).toString()}updateColumnSql(e){const{table:a,columnName:t,newColumnName:r,columnType:n,isNotNull:E,isAutoIncrement:s,comment:o,defaultValue:c,oldRow:_}=e,A=`ALTER TABLE "${a}"`,m=E?"SET NOT NULL":"DROP NOT NULL";return new i(`${A} ALTER COLUMN "${t}" TYPE ${n};`,`
`).if(o&&o!=_.comment,`${A} ALTER COLUMN "${t}" COMMENT '${o}';`).if(E!=_.isNotNull,`${A} ALTER COLUMN "${t}" ${m};`).if(t!=r,`${A} RENAME COLUMN "${t}" TO "${r}";`).toString()}showCollations(){return""}showChecks(e,a){return""}pingDataBase(e,a){return e?`use "${a}"."${e}"`:"select 1"}updateTable(e){const{table:a,newTableName:t,comment:r,newComment:n,collation:E,newCollation:s}=e;let o="";return n&&n!=r&&(o=`ALTER TABLE \`${a}\` COMMENT = '${n}';`),s&&s!=E&&(o+=`ALTER TABLE \`${a}\` collate = '${s}';`),t&&a!=t&&(o+=`ALTER TABLE \`${a}\` RENAME TO \`${t}\`;`),o}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE \`',table_schema,'\`.\`',TABLE_NAME, '\`;') trun FROM INFORMATION_SCHEMA.TABLES where  table_schema ='${e}' and TABLE_TYPE<>'VIEW';`}createDatabase(e){return"CREATE DATABASE $1;"}showTableSource(e,a){return`select get_ddl('table', '${e}.${a}') "Create Table";`}showViewSource(e,a){return`select get_ddl('view', '${e}.${a}') "Create View";`}showProcedureSource(e,a){return`select get_ddl('PROCEDURE', '${e}.${a}()') "Create Procedure";`}showFunctionSource(e,a){return`select get_ddl('FUNCTION', '${e}.${a}()') "Create Function";`}showColumns(e,a){return`SELECT 
        c.COLUMN_NAME "name",
        DATA_TYPE "type",
        CHARACTER_MAXIMUM_LENGTH "maximum_length",
        IS_IDENTITY "key",
...
ORDER BY "view_group",name`}showSequences(e){return`SELECT sequence_name AS name, last_value AS sequence 
    ...
    content TEXT
        ...
        FROM information_schema.columns c
        WHERE c.table_schema = '${e}' AND c.table_name = '${a}' 
        ORDER BY c.ORDINAL_POSITION;`:`SELECT t1.*,t1.pk "key",t1.dflt_value "defaultValue" FROM PRAGMA_TABLE_INFO('${a}') t1;`}updateColumn(e,a){const{name:t,type:r}=a;return`-- change column type
ALTER TABLE ${e} 
    ALTER COLUMN ${t} TYPE ${r};
-- change column name
ALTER TABLE ${e} 
    RENAME COLUMN ${t} TO ${t};`}updateColumnSql(e){return new d(e).genAlterSQL()}}class U extends S{showVersion(){return"select version() as server_version;"}showDatabases(){return"show databases"}showTables(e,a){return`show tables in ${e}`}showColumns(e,a){return`describe ${e}.${a};`}showViews(e,a){return`show views in ${e}`}}class k extends S{showVersion(){return"SELECT node_version as server_version FROM system.runtime.nodes;"}pingDataBase(e){return e?`use ${e}`:null}showDatabases(){return"show catalogs"}}class q extends S{showVersion(){return"select release_version from system.local;"}showUsers(){return'SELECT role as "user" FROM system_auth.roles;'}pingDataBase(e){return e?`use ${e}`:null}createDatabase(e){return`CREATE KEYSPACE $1
WITH REPLICATION = { 
    ...
    update_time DATE,
    content TEXT
);`}}class j extends d{otherPart(){const{oldRow:e,isAutoIncrement:a}=this.param;return!e.isAutoIncrement&&a?`${this.afterColumnPrefix} SET GENERATED ALWAYS as identity;`:e.isAutoIncrement&&!a?`${this.afterColumnPrefix} DROP GENERATED;`:null}commentPart(){const{oldRow:e,table:a,columnName:t,comment:r}=this.param;if(r!=e.comment)return`COMMENT ON COLUMN "${a}"."${t}" is '${r}';`}changeTypePart(){const{oldRow:e,columnType:a}=this.param;return e.type==a?"":`${this.afterColumnPrefix} SET DATA TYPE ${a};`}}class f extends S{showVersion(){return'SELECT SERVICE_LEVEL as "server_version" FROM SYSIBMADM.ENV_INST_INFO;'}showUsers(){return`SELECT GRANTEE "user" FROM syscat.dbauth WHERE GRANTEETYPE='U';`}pingDataBase(e){return e?`set SCHEMA ${e}`:null}showDatabases(){return'select schemaname "database" from syscat.schemata'}showTableSource(e,a){return null}showTables(e,a){return`select tabname "name", remarks "comment" from syscat.tables where tabschema='${a}' and TYPE='T' order by tabname`}showColumns(e,a){return`select COLUMN_NAME "name",
        DATA_TYPE "type",
        IS_NULLABLE "nullable",
        NUMERIC_PRECISION "precision",
        NUMERIC_SCALE "scale",
        ...
        where
            TABLE_SCHEMA = '${e}'
            and TABLE_NAME = '${a}'
        order by ORDINAL_POSITION;`}updateColumnSql(e){return new j(e).genAlterSQL()}showAllForeignKeys(){return`SELECT 
 ...
 ORDER BY elapsed_time_sec DESC`}}class J extends M{tableTemplate(){return`CREATE TABLE table_name(  
...
PROPERTIES (
    "replication_allocation" = "tag.location.default: 1"
        ...
        FROM information_schema.columns c
        WHERE c.table_schema = '${e}' AND c.table_name = '${a}' 
        ORDER BY c.ORDINAL_POSITION;`}}class X extends N{showVersion(){return"SELECT SERVER_VERSION();"}createIndex(e){let a=`${e.type||"key"}`;return a.match(/BTREE/i)&&(a="key"),`ALTER TABLE ${e.table} ADD ${a} (\`${e.column||"$1"}\`)`}dropIndex(e,a){return`ALTER TABLE ${e} DROP INDEX \`${a}\``}showIndex(e,a){return`SELECT column_name "column_name",index_name "index_name",index_type "index_type",non_unique=0 "isUnique" FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema='${e}' and table_name='${a}';`}addColumn(e,a){const t=a?` AFTER \`${a}\``:"";return`ALTER TABLE ${e} 
    ADD COLUMN $1 [type]$2 COMMENT '$3'${t};`}createUser(){return`CREATE USER '$1'@'%' IDENTIFIED BY 'password$2';
-- Grant select privilege to all databases;
GRANT SELECT ON *.* TO '$1'@'%' WITH GRANT OPTION;
-- Grant all privileges to all databases;
GRANT ALL PRIVILEGES ON *.* TO '$1'@'%' WITH GRANT OPTION;`}updateUser(e){return`update mysql.user set 
    ...
    where User = '${e}';
FLUSH PRIVILEGES;
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")`}updateColumn(e,a){var n,E;const{nullable:t,extra:r}=a;return this.updateColumnSql({table:e,...a,isNotNull:t!="YES",isAutoIncrement:(E=(n=r==null?void 0:r.toLowerCase())==null?void 0:n.includes)==null?void 0:E.call(n,"auto_increment")})}updateColumnSql(e){const{table:a,name:t,columnName:r=t,type:n,unsigned:E,zerofill:s,useCurrentTimestamp:o,isNotNull:c,isAutoIncrement:_,comment:A,defaultValue:m,character_set_name:R,collation_name:C}=e,O=!F(n)&&!(n!=null&&n.match(/json/i));return new i(`ALTER TABLE \`${a}\``).append(`
	CHANGE \`${r}\` \`${t}\` ${n}`).if(E=="1","UNSIGNED").if(s=="1","ZEROFILL").if(o,"ON UPDATE CURRENT_TIMESTAMP").if(O&&R,`CHARACTER SET ${R}`).if(O&&C,`COLLATE ${C}`).if(c,"NOT NULL").if(_,"AUTO_INCREMENT").if(A,`COMMENT '${A}'`).if(u(m)&&!c,"DEFAULT NULL").if(!u(m),`DEFAULT ${m=="CURRENT_TIMESTAMP"?m:`${L(m,n)}`}`).append(";").toString()}showCollations(){return null}pingDataBase(e){return e?`use \`${e}\``:null}updateTable(e){const{table:a,newTableName:t,comment:r,newComment:n,collation:E,newCollation:s}=e;let o="";return n&&n!=r&&(o=`ALTER TABLE \`${a}\` COMMENT = '${n}';`),s&&s!=E&&(o+=`ALTER TABLE \`${a}\` collate = '${s}';`),t&&a!=t&&(o+=`ALTER TABLE \`${a}\` RENAME TO \`${t}\`;`),o}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE \`',table_schema,'\`.\`',TABLE_NAME, '\`;') trun FROM INFORMATION_SCHEMA.TABLES where  table_schema ='${e}' and TABLE_TYPE<>'VIEW';`}createDatabase(e){return"CREATE DATABASE $1;"}showTableSource(e,a){return`SHOW CREATE TABLE \`${e}\`.\`${a}\`;`}showPartitions(e,a){return`SELECT 
PARTITION_NAME "name",PARTITION_METHOD "strategy",PARTITION_EXPRESSION "columnName",
PARTITION_DESCRIPTION "value",TABLE_ROWS "rows",DATA_LENGTH "length"
...
SELECT * FROM `}createDatabase(e){return`CREATE SCHEMA ${e}$1;`}}class Z extends f{showVersion(){return"SELECT SYSTEM_VALUE_NAME, CURRENT_CHARACTER_VALUE FROM QSYS2.SYSTEM_VALUE_INFO WHERE SYSTEM_VALUE_NAME IN ('QSRLNBR', 'QMODEL');"}showUsers(){return`SELECT AUTHORIZATION_NAME AS "user" FROM QSYS2.USER_INFO WHERE STATUS = 'ENABLED';`}pingDataBase(e){return e?`SET SCHEMA ${e}`:null}showDatabases(){return'SELECT SCHEMA_NAME AS "database" FROM QSYS2.SYSSCHEMAS;'}showTables(e,a){return`SELECT TABLE_NAME AS "name", TABLE_TEXT AS "comment" FROM QSYS2.SYSTABLES WHERE TABLE_SCHEMA='${a}' AND TABLE_TYPE='T' ORDER BY TABLE_NAME;`}showColumns(e,a){return`SELECT COLUMN_NAME AS "name",
                ...
                ORDER BY TOTAL_CPU_TIME DESC;`}}class h extends M{showDatabases(){return"show databases"}showTables(e){return`show tables from ${e}`}showColumns(e,a){return`show columns from ${a}`}showViews(e){return`show views from ${e}`}showIndexes(e,a){return`show indexes from ${a}`}showFunctions(e){return`show functions from ${e}`}showProcedures(e){return`show procedures from ${e}`}showTriggers(e,a){return`show triggers from ${a}`}}class z extends ${showVersion(){return"select replace(version(), 'KingbaseES ', '');"}showViewSource(e,a,t){return t?`SELECT 
    'CREATE MATERIALIZED VIEW ' || schemaname || '.' || matviewname || ' AS ' ||
    definition as mview_ddl
FROM sys_matviews
WHERE schemaname = '${e}' 
    AND matviewname = '${a}';`:`SELECT 
    'CREATE OR REPLACE VIEW ' || schemaname || '.' || viewname || ' AS ' || 
    definition as view_ddl
FROM sys_views 
WHERE schemaname = '${e}' 
    AND viewname = '${a}';`}triggerTemplate(e){return`CREATE TRIGGER trigger_name$1
[BEFORE/AFTER] INSERT ON ${e??"[table_name]$2"}
FOR EACH ROW BEGIN
    $3
END;`}}class ee extends b{showDatabases(){return"list"}showTables(e){return`SELECT name, type FROM sqlite_master 
                WHERE type='table' 
                AND name NOT LIKE '_cf%' 
                ORDER BY type ASC, name ASC;`}}class ae extends I{showVersion(){return"SELECT VERSION() as server_version"}createIndex(e){return null}variableList(){return"SHOW ALL"}statusList(){return`SELECT
        'db_numbackends' AS db,
        pg_stat_get_db_numbackends(datid) AS status
      FROM
        pg_stat_database
...
AS $$
    SELECT 1
$$ LANGUAGE sql;`}}class te extends S{showVersion(){return"select version() as server_version;"}showDatabases(){return"SHOW CATALOGS"}pingDataBase(e,a){return`USE ${e}`}showTables(e,a){return`SELECT table_name FROM information_schema.tables
...
CASE WHEN tc.constraint_type = 'PRIMARY KEY' THEN 'YES' ELSE 'NO' END AS key
    ...
    WHERE c.table_schema = '${e}' AND c.table_name = '${a}'
    ORDER BY c.ordinal_position;`}createDatabase(e){return`CREATE CATALOG $1${e};`}dropDatabase(e){return`DROP CATALOG ${e};`}}class ne extends I{showTables(e,a){return`SELECT t.table_name "name", pg_catalog.obj_description(pgc.oid, 'pg_class') "comment",pgc.reltuples "table_rows"
FROM information_schema.tables t
JOIN pg_catalog.pg_class pgc ON t.table_name = pgc.relname 
JOIN pg_catalog.pg_namespace pgn ON pgn.oid=pgc.relnamespace and pgn.nspname=t.table_schema
WHERE t.table_type='BASE TABLE'
AND t.table_schema='${a}' order by t.table_name;`}processList(){return`SELECT
          ...
          ORDER BY
            a.pid ASC,
            c.relname ASC`}statusList(){return null}}class Ee extends S{showVersion(){return"SELECT PARAM_VALUE FROM EXA_METADATA WHERE PARAM_NAME = 'databaseProductVersion';"}showDatabases(){return"SELECT SCHEMA_NAME FROM EXA_SCHEMAS ORDER BY SCHEMA_NAME;"}showTables(e,a,t){return`SELECT TABLE_NAME FROM SYS.EXA_ALL_TABLES WHERE TABLE_SCHEMA = '${a}' ORDER BY TABLE_NAME;`}showViews(e,a){return`SELECT VIEW_NAME FROM SYS.EXA_ALL_VIEWS WHERE VIEW_SCHEMA = '${a}' ORDER BY VIEW_NAME;`}pingDataBase(e){return e?`OPEN SCHEMA ${e};`:null}showColumns(e,a,t){return`SELECT 
            COLUMN_NAME as "name",
            COLUMN_TYPE as "type",
...
END;`}showUsers(){return'SELECT USER_NAME "user" FROM SYS.EXA_ALL_USERS ORDER BY USER_NAME;'}userTemplate(){return"CREATE USER user_name IDENTIFIED BY 'password';"}truncateDatabase(e){return`SELECT 'TRUNCATE TABLE ' || TABLE_SCHEMA || '.' || TABLE_NAME || ';' 
        ...
        current_schema as current_database`}createDatabase(){return"CREATE SCHEMA $1"}}function le(T,e){switch(T||(T=l.MYSQL),T){case l.MYSQL:return e?new h:new M;case l.MARIA_DB:return new P;case l.DORIS:return new J;case l.MYSQL_COMPATIBLE:return new h;case l.KINGBASE:return new z;case l.COCKROACH:return new ne;case l.PG:return e?new I:new $;case l.REDSHIFT:return new x;case l.GAUSS_DB:return new ae;case l.SQLServer:return new W;case l.D1:return new ee;case l.libSQL:case l.SQLITE:return new b;case l.DUCK_DB:return new V;case l.SNOWFLAKE:return new v;case l.CLICKHOUSE:return new H;case l.DM:return new y;case l.ORACLE:return new g;case l.CASSANDRA:return new q;case l.BIG_QUERY:return new Q;case l.PRESTO:case l.TRINO:return new k;case l.HIVE:return new U;case l.DATABRICKS:return new te;case l.ATHENA:return new K;case l.DB2:return new f;case l.AS400:return new Z;case l.MONGO_DB:return new Y;case l.NEO4J:return new G;case l.TDengine:return new X;case l.EXASOL:return new Ee}return new S}export{le as g};
